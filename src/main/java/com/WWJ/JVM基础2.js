...
*
* 3、解析阶段:将指向常量池中的符号引用替换为指向内存的直接引用
* 符号引用就是在字节码文件中使用编号(cp_info#6)，来访问常量池中的内容(需要一层层的索引找到最终的字符串名称,就只是名称)
* 直接引用不再使用索引一层层找，而是在常量池中直接使用内存中地址进行访问具体的数据
* 例如:1 invokespecial #1 <java/lang/Object.<init>> //调用父类Object的构造方法
  原先是常量池中的是Object类的字符串名称即java/lang/Object
  加载到内存后，将常量池中#1对应的直接修改为 Object构造方法的内存地址，@0x00000007c0000f28
...
注意！！！
字节码指令在类加载到内存之后，会被完整保存到方法区中,是不会被修改的
链接阶段的解析，只是修改常量池，将原先的字符串形式的符号引用，修改为内存地址的直接引用
例如:
0: ldc #2 <java/lang/Object> // 将常量池中编号为2的符号引用(java/lang/Object)放到操作数栈中
常量池中的#2对应的数据为，字符串名称的java/lang/Object，而且可能需要一层层的索引查找
在进行类加载器加载到内存中之后，
常量池中#2对应的数据直接为Object类的内存地址，不用一层层的找

注意！！！
这种在解析阶段就将那个将符号引用修改为直接引用的称为静态解析
这种编号和内存地址的映射关系时永久的，直到方法被回收

注意！！！
一般，只有字节码指令指向静态方法、构造方法、私有方法和final修饰的方法时，可以时静态解析
//初始化阶段
/*
